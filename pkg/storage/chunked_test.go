...
var _ = Describe("chunkedSecrets", func() {
	runTests := func(releaseNameLen int) {
		...
		var _ = Describe("List", func() {
			BeforeEach(func() {
				releases := []*release.Release{
					genRelease(strings.Repeat("a", releaseNameLen), 1, release.StatusSuperseded, nil, chunkSize/2),
					genRelease(strings.Repeat("a", releaseNameLen), 2, release.StatusSuperseded, nil, chunkSize/2),
					genRelease(strings.Repeat("a", releaseNameLen), 3, release.StatusSuperseded, nil, chunkSize*2),
					genRelease(strings.Repeat("a", releaseNameLen), 4, release.StatusDeployed, nil, chunkSize*2),

					genRelease(strings.Repeat("b", releaseNameLen), 1, release.StatusSuperseded, nil, chunkSize*2),
					genRelease(strings.Repeat("b", releaseNameLen), 2, release.StatusSuperseded, nil, chunkSize*2),
					genRelease(strings.Repeat("b", releaseNameLen), 3, release.StatusDeployed, nil, chunkSize/2),
				}
			...
			It("should list releases by name", func() {
				aReleases, err := chunkedDriver.List(func(rel *release.Release) bool {
					return rel.Name == strings.Repeat("a", releaseNameLen)
				})
				Expect(err).ToNot(HaveOccurred())
			...
			It("should return an empty list if no releases match", func() {
				cReleases, err := chunkedDriver.List(func(rel *release.Release) bool {
					return rel.Name == strings.Repeat("c", releaseNameLen)
				})
				Expect(err).ToNot(HaveOccurred())
		...
		var _ = Describe("Query", func() {
			BeforeEach(func() {
				releases := []*release.Release{
					...
					genRelease(strings.Repeat("b", releaseNameLen), 1, release.StatusSuperseded, nil, chunkSize*2),
					genRelease(strings.Repeat("b", releaseNameLen), 2, release.StatusSuperseded, nil, chunkSize*2),
					genRelease(strings.Repeat("b", releaseNameLen), 3, release.StatusDeployed, map[string]string{"key1": "val1"}, chunkSize/2),
				}
				for _, rel := range releases {
					Expect(chunkedDriver.Create(releaseKey(rel), rel)).To(Succeed())
				}
			...
			It("should succeed if no matched release is too large", func() {
				...
				actual, err := maxReadDriver.Query(map[string]string{"name": strings.Repeat("a", releaseNameLen), "version": "1"})
				Expect(err).ToNot(HaveOccurred())
			...
			It("should fail if any matched release is too large", func() {
				...
				actual, err := maxReadDriver.Query(map[string]string{"name": strings.Repeat("a", releaseNameLen)})
				Expect(err).To(MatchError(ContainSubstring("release too large")))
				Expect(actual).To(BeNil())
			})
	...
	for _, releaseNameLen := range []int{1, maxLabelLength} {
		runTests(releaseNameLen)
	}
})
